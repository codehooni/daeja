...
class ParkingDetailBottomSheet extends StatelessWidget {
  ...
  const ParkingDetailBottomSheet({super.key, required this.parkingLot});
  ...
  static void show(BuildContext context, ParkingLot parkingLot) {
    showModalBottomSheet(
      ...
      builder: (context) => ParkingDetailBottomSheet(parkingLot: parkingLot),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return VStack([
          ...
          VStack([
            ...
            // 요금 정보
            if (parkingLot.basePrice != null && parkingLot.unitTime != null && parkingLot.unitPrice != null) ...[
              _buildPricingInfo(),
              12.heightBox,
            ...
            // 버튼 영역
            if (parkingLot.type == ParkingLotType.valet)
              // 발렛: 길찾기 40% + 예약하기 60%
              HStack([
                // 길찾기 버튼 (40%)
                HStack([
                      ...
                      6.widthBox,
                      '길찾기'.text.white.bold.size(14).make(),
                    ])
                    .centered()
                    .p12()
                    .box
  ...
  Widget _buildAvailabilityStatus() {
    ...
    ).pSymmetric(v: 12, h: 12).box.roundedSM.color(bgColor).make();
  }
  ...
  Widget _buildPricingInfo() {
    ...
    if (isValet) {
      ...
      final day1Exit = today.add(const Duration(days: 0, hours: 23, minutes: 59));
      final day2Exit = today.add(const Duration(days: 1, hours: 23, minutes: 59));
      final day3Exit = today.add(const Duration(days: 2, hours: 23, minutes: 59));
    ...
    return VStack([
      ...
      VStack([
        HStack([
          ...
          Spacer(),
          '${basePrice.toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}원${isValet ? '' : ' (최초 ${_formatDuration(unitTime)})'}'.text.size(13).semiBold.make(),
        ]),
        4.heightBox,
        HStack([
          (isValet ? '주차 요금' : '추가 요금').text.size(13).gray600.make(),
          Spacer(),
          '${unitPrice.toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}원 / ${_formatDuration(unitTime)}'.text.size(13).semiBold.make(),
        ]),
        8.heightBox,
        Container(
          ...
          child: VStack([
            ...
            HStack([
              ...
              Spacer(),
              '${fee1.toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}원'.text.size(12).semiBold.make(),
            ]),
            HStack([
              label2.text.size(12).gray600.make(),
              Spacer(),
              '${fee2.toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}원'.text.size(12).semiBold.make(),
            ]),
            HStack([
              label3.text.size(12).gray600.make(),
              Spacer(),
              '${fee3.toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}원'.text.size(12).semiBold.make(),
            ]),
          ]),
        ),
      ]).pOnly(left: 28),
